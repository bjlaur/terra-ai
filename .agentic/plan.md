...
- **Git:** Repo at `/home/agent/git/terra-ai`. Default branch: `main`. Branches: `release-0.0.1` (merged), `release-0.0.2` (current), `agent1/sopeL-native-cleanup` (done, needs merge), `agent1/carry-over-0.0.2`, `agent1/test-profiling` (current active).
- **Agent name:** OWL
...
|---|---|---|---|
| `.listprompts` | `.listprompts` | Select all rows from `prompts`, ordered alphabetically by trigger. | PMs each as `#N trigger: response`. If empty: "No prompts configured." |
...
| `.effort` | `.effort [low\|medium\|high\|xhigh\|max]` | Sets the reasoning effort level for AI responses (passed to provider as `effort` parameter). Without argument, shows current level. | "Effort level: high" |
