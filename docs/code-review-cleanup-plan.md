...
17. **Logging is correlated, levelled, and intentionally routed.** Assign one unique correlation ID when an inbound IRC event begins and propagate it through plugin routing, `TerraAI`, provider calls, tool calls, retries, errors, and the final response.
    ...
    - DEBUG and all higher-severity TerraAI records go to a dedicated rotating `terra-ai.log`, making it the complete operational log: DEBUG, INFO, WARNING, ERROR, and full stack traces. OpenRouter TRACE records go to a separate rotating `openrouter-trace.log`; TRACE is filtered out of `terra-ai.log`.
