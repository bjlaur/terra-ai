...
- Tool enable/disable/list commands were intended as admin-managed server-wide policy, but the current implementation has no admin guard and incorrectly stores overrides by `(server, nick)` in `UserStore`. Correcting this requires moving ownership out of user commands and replacing the tool-settings schema; do not preserve the accidental per-user semantics.
...
3. **PMs are conversational by default.** Bare PM text routes to AI. Registered PM commands execute once as commands; unknown prefixed PM text routes to AI once with the complete prefixed input preserved as conversation context.
...
12. **Error handling follows one explicit decision table.** An error is caught at the nearest boundary that can genuinely recover or add useful context; translated exceptions preserve their original cause.
    ...
    - An unexpected but recoverable exception is handled, logged with its full stack trace, and emitted to IRC as a separate `Error:` message. Normal processing may continue only when state is known to remain valid. All such continuing-error notices pass through one narrow, dedicated reporting method rather than the terminal plugin error path or ad hoc `bot.say()` calls. This preserves their distinct “report, then continue” semantics and provides one future configuration point; adding that configuration is not part of this phase.
...
17. **Logging is correlated, levelled, and intentionally routed.** Assign one unique correlation ID when an inbound IRC event begins and propagate it through plugin routing, `TerraAI`, provider calls, tool calls, retries, errors, and the final response.
    ...
    - DEBUG and all higher-severity TerraAI records go to a dedicated rotating `terra-ai.log`, making it the complete operational log: DEBUG, INFO, WARNING, ERROR, and full stack traces. OpenRouter TRACE records go to a separate rotating `openrouter-trace.jsonl`; TRACE is filtered out of `terra-ai.log`.
    - INFO, WARNING, and ERROR remain visible through SOPEL stderr. DEBUG and TRACE are file-only during normal operation. Every logged exception includes `exc_info=True` so `terra-ai.log` contains the complete stack trace.
    ...
    - Trace payloads may contain private conversation and tool data because this is a private diagnostic mode. Do not pass authorization headers to logging, and replace the configured provider API key if it appears in final formatted output. Do not build a general-purpose secret-classification system for this private deployment.
